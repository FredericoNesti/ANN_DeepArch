    ...
    def cd1(self,visible_trainset, n_iterations=10000):
        ...
        for it in range(n_iterations):
            ...
            if it % splits == 0:
                ...
                if len(loss_buffer) > 0:
                    loss_list.append(sum(loss_buffer) / len(loss_buffer))
                loss_buffer = []
	    ...
	    # [TODO TASK 4.1] run k=1 alternating Gibbs sampling : v_0 -> h_0 ->  v_1 -> h_1.
            ...
            if it % self.print_period == 0:
                prob, h_0 = self.get_h_given_v(v_minibatch)
                v_k, v_k_sample = self.get_v_given_h(h_0)
                loss = np.linalg.norm(v_minibatch - v_k_sample)
                loss_buffer.append(loss/len(v_minibatch))
                print ("iteration=%7d, epoch=%7d recon_loss=%4.4f"%(it, epochs, loss))

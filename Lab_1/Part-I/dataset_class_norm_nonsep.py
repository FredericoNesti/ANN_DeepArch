...
class Dataset():
    ...
    def __init__(self, n, mA, sigmaA, mB, sigmaB, batchSize,case=1, shuffleDataset=True):
        ...
        if case == 1:
            ...
            selected_indexes = list(np.argwhere(self.classA[1,:]>0).flatten()[:int(n/10*4)]) + list(np.argwhere(self.classA[1,:]<0).flatten()[:int(n/10)])
            self.testclassA = self.classA[:,selected_indexes]
            self.classA = np.delete(self.classA,selected_indexes,axis=1)
            testset = self.testclassA
    ...
    def printDataset(self, Net):
        """
        :return: it prints the dataset with the boarder separation of weights
        """
        x = np.arange(min(self.X[:, 0]), max(self.X[:, 1]), 0.005)
        seperation=[]
        for i,l1 in enumerate(x):
            for j,l2 in enumerate(x):
                if Net.Output(np.array([l1,l2]))[0][0] < 0.05 and Net.Output(np.array([l1,l2]))[0][0] > -0.05:
                    seperation.append([l1,l2])
        if len(seperation) == 0:
            return 0
        seperation = np.array(seperation)
        plt.plot(seperation[:,0], seperation[:,1], 'k.', label="separation line")

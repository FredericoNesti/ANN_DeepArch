...
def d_Activation_Func(X_vec):
    # Derivative of the Activatoion Function
...
class Perceptron():
   ...
   # def __init__(self,inputs,targets,neurons_structure,train_step):
    ...
    def Backprop_train(self,inputs,targets):
        ...
        for i in range(self.no_layers-2,-1,-1):
            ...
            self.all_deltas.append((self.all_weights[i].T @ self.all_deltas[-1] )*( d_Activation_Func(self.all_signals[i])))

            self.all_updates.append(-self.train_step*( self.all_deltas[-1] @ self.all_signals[i] ))
            
        self.all_updates.append(-self.train_step*( self.all_deltas[-1] @ inputs ))
        
        for i in range(self.no_layers):
            self.all_weights[i] += self.all_updates[i]
